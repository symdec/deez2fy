...
def progress_print(
    index: int, nb_songs: int, song: str, artist: str, is_ok: bool
) -> None:
    """Prints a progress message."""
    status = "OK" if is_ok else "NOK"
    line_start = f"({index + 1}/{nb_songs}) [{song}] by [{artist}] :"
    if len(line_start) > PROGRESS_PRINT_LINE_LENGTH: # if line is too long
        # print the first part
        print(line_start[: PROGRESS_PRINT_LINE_LENGTH - 2])
        # store the second part
        line_start = line_start[PROGRESS_PRINT_LINE_LENGTH - 2 :]
    # pad the last line with dots and add status
